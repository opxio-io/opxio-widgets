...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ── Page 2 — System ────────────────────────────────────────────────────── */
  .sys-header{display:flex;justify-content:space-between;align-items:baseline;padding-bottom:20px;border-bottom:1.5px solid var(--black);margin-bottom:28px}
  .sys-heading{font-family:var(--fh);font-size:26px;font-weight:700;color:var(--black);letter-spacing:-.01em}
  .sys-total{font-family:var(--fh);font-size:22px;font-weight:700;color:var(--black)}
  .sys-subtitle{font-size:12.5px;color:var(--muted);line-height:1.6;margin-bottom:0;max-width:500px}

  .os-row{display:grid;grid-template-columns:1fr auto;align-items:start;gap:20px;padding:20px 0;border-bottom:1px solid var(--border)}
  .os-row:last-child{border-bottom:none}
  .os-name{font-family:var(--fh);font-size:14px;font-weight:700;color:var(--black);margin-bottom:4px}
  .os-badge{font-size:10px;color:var(--muted);font-style:italic;margin-left:8px;font-weight:400}
  .os-price{font-family:var(--fh);font-size:14px;font-weight:700;color:var(--black);white-space:nowrap;text-align:right}
  .os-price-note{font-size:10px;color:var(--muted);text-align:right;margin-top:2px}
  .os-desc{font-size:12px;color:var(--muted);font-style:italic;margin-bottom:10px;line-height:1.55}
  .modules-label{font-family:var(--fh);font-size:8.5px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin-bottom:5px}
  .modules-inline{font-size:12px;color:var(--fg);line-height:1.65}
  .modules-inline span:not(:last-child)::after{content:' ·';color:var(--muted)}

  .section-divider{height:1.5px;background:var(--black);margin:24px 0 16px}
  .addons-section-heading{font-family:var(--fh);font-size:10px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin-bottom:12px}
  .addon-row{display:grid;grid-template-columns:1fr auto;align-items:baseline;gap:16px;padding:10px 0;border-bottom:1px solid var(--border)}
  .addon-row:last-child{border-bottom:none}
  .addon-row-name{font-family:var(--fh);font-size:12.5px;font-weight:600;color:var(--black)}
  .addon-row-desc{font-size:11px;color:var(--muted);margin-top:2px;line-height:1.5}
  .addon-row-price{font-family:var(--fh);font-size:12.5px;font-weight:700;color:var(--black);white-space:nowrap;text-align:right}
  .addon-row-cad{font-size:10px;color:var(--muted);text-align:right}
...
<div class="page">
  <div class="page-body">
    ...
    <div class="sys-header">
      ...
      ${coreFee ? `<div class="sys-total">${fmt(coreFee)}</div>` : ''}
    </div>
    ...
    }).join('') : `<div class="os-row"><div class="os-name" style="color:var(--muted);font-style:italic">System scope confirmed on discovery call.</div></div>`}

    <!-- Add-ons in this proposal + recommended -->
    ${addons_now.length || addons_later.length ? `
    <div class="section-divider"></div>
    <div class="addons-section-heading">Add-Ons${addons_now.length ? ' — Included in This Proposal' : ' — Available Any Time'}</div>
    ${addons_now.map(item => {
      const isObj = typeof item === 'object' && item !== null;
      const name  = isObj ? (item.name || '') : String(item);
      const lib   = ADDON_LIBRARY[name] || {};
    ...
    ${addons_now.length && addons_later.length ? `<div style="margin-top:16px"><div class="addons-section-heading">Also Available Any Time</div>` : ''}
    ${addons_later.length ? `<div style="font-size:12px;color:var(--muted);line-height:1.9">
      ${addons_later.map(n => `<span style="white-space:nowrap">${escape(n)}</span>`).join('<span style="color:var(--border);margin:0 6px">·</span>')}
    </div>${addons_now.length && addons_later.length ? '</div>' : ''}` : ''}
    ` : ''}
