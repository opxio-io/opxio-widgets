...
function renderCustomBlocks(blocks) {
  ...
  for (const block of blocks) {
    ...
    if (type !== 'bulleted_list_item' && bulletBuffer.length > 0) {
      html += `<ul class="block-list">${bulletBuffer.map(t => `<li>${escape(t)}</li>`).join('')}</ul>`;
      bulletBuffer = [];
    }

    if (type === 'heading_1') {
      html += `<h2 class="block-h1">${escape(text)}</h2>`;
    } else if (type === 'heading_2') {
      html += `<h3 class="block-h2">${escape(text)}</h3>`;
    } else if (type === 'bulleted_list_item') {
      bulletBuffer.push(text);
    } else {
      // paragraph
      html += `<p class="section-lead">${escape(text)}</p>`;
    }
  }
...
const CSS = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  :root{
    --bg:#0D0D0D;--surface:#111111;--lime:#AAFF00;--lime-dim:rgba(170,255,0,.5);--lime-soft:rgba(170,255,0,.08);--lime-border:rgba(170,255,0,.2);
    --border:rgba(255,255,255,.06);--border2:rgba(255,255,255,.1);--text:rgba(255,255,255,.87);--muted:rgba(255,255,255,.45);--subtle:rgba(255,255,255,.2);
    --fd:'Satoshi',sans-serif;
  }
  html{font-size:15px;background:#0D0D0D}
  body{font-family:var(--fd);color:var(--text);background:#0D0D0D;-webkit-font-smoothing:antialiased}
  .page{width:860px;background:var(--bg);margin:32px auto;position:relative}
  @media print{
    *{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
    body{background:#0D0D0D!important;margin:0;padding:0}
    ...
    */
    ...
    .phase2-grid{display:grid;grid-template-columns:1fr}
  ...
  .cover{background:#000;min-height:100vh;display:flex;flex-direction:column;padding:56px 64px;position:relative;overflow:hidden}
  .cover::before{content:'';position:absolute;top:-180px;right:-180px;width:520px;height:520px;border-radius:50%;border:1px solid rgba(170,255,0,.08);pointer-events:none}
  .cover::after{content:'';position:absolute;top:-80px;right:-80px;width:300px;height:300px;border-radius:50%;border:1px solid rgba(170,255,0,.12);pointer-events:none}
  ...
  .logo-mark{font-family:var(--fd);font-size:13px;font-weight:700;letter-spacing:.18em;color:var(--text);text-transform:uppercase;display:flex;align-items:center}
  .cover-ref{font-size:11px;color:rgba(255,255,255,.25);letter-spacing:.08em;text-align:right;line-height:1.8}
  .cover-content{padding:80px 0 48px}
  .cover-eyebrow{font-size:9px;font-weight:700;letter-spacing:.18em;color:var(--lime-dim);text-transform:uppercase;margin-bottom:24px}
  .cover-title{font-family:var(--fd);font-size:68px;font-weight:900;line-height:1.0;color:#fff;margin-bottom:8px;letter-spacing:-.03em}
  .cover-title span{color:var(--lime);display:block}
  .cover-subtitle{font-size:17px;font-weight:400;color:var(--muted);margin-top:24px;line-height:1.6}
  .cover-divider{width:48px;height:2px;background:var(--lime);margin:32px 0}
  .cover-meta{display:grid;grid-template-columns:1fr 1fr;gap:24px;padding-top:32px;border-top:1px solid rgba(255,255,255,.05)}
  .cover-meta-item label{display:block;font-size:10px;font-weight:700;letter-spacing:.14em;color:rgba(255,255,255,.25);text-transform:uppercase;margin-bottom:6px}
  .cover-meta-item span{display:block;font-size:14px;font-weight:400;color:rgba(255,255,255,.75)}
  .page-header-strip{display:flex;justify-content:space-between;align-items:center;padding:14px 64px;border-bottom:1px solid rgba(255,255,255,.05);background:rgba(255,255,255,.02)}
  .page-header-strip .logo{font-family:var(--fd);font-size:12px;font-weight:900;letter-spacing:.16em;color:rgba(255,255,255,.3);text-transform:uppercase;display:flex;align-items:center}
  .page-header-strip .doc-label{font-size:10px;font-weight:600;letter-spacing:.12em;color:rgba(255,255,255,.15);text-transform:uppercase}
  .inner{padding:52px 64px 60px}
  .section-block{margin-bottom:52px;page-break-inside:avoid;break-inside:avoid}
  .section-eyebrow{font-size:9px;font-weight:700;letter-spacing:.18em;color:var(--lime-dim);text-transform:uppercase;margin-bottom:10px;display:flex;align-items:center;gap:10px}
  .section-eyebrow::after{content:'';flex:1;height:1px;background:var(--subtle);max-width:120px}
  .section-title{font-family:var(--fd);font-size:36px;font-weight:900;color:#fff;line-height:1.1;letter-spacing:-.03em;margin-bottom:20px}
  .section-lead{font-size:14px;font-weight:400;color:var(--muted);line-height:1.75;max-width:600px}
  .section-lead+.section-lead{margin-top:14px}
  .context-label{font-size:9px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:rgba(170,255,0,.4);margin-top:28px;margin-bottom:6px}
  .context-label.first{margin-top:0}
  .section-lead strong{font-weight:600;color:rgba(255,255,255,.75)}
  .summary-grid{display:grid;grid-template-columns:1fr 1fr;gap:1px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);margin:36px 0;border-radius:0;overflow:hidden}
  .summary-item{padding:20px 24px;border-bottom:1px solid rgba(255,255,255,.06);border-right:1px solid rgba(255,255,255,.06);background:#111}
  .summary-item:nth-child(even){border-right:none}
  .summary-item:nth-last-child(-n+2){border-bottom:none}
  .summary-item label{display:block;font-size:9px;font-weight:700;letter-spacing:.12em;color:rgba(255,255,255,.25);text-transform:uppercase;margin-bottom:6px}
  .summary-item span{font-size:14px;font-weight:400;color:rgba(255,255,255,.65);line-height:1.4}
  .summary-item span.hl{color:#AAFF00;font-weight:600}
  .block-h1{font-size:20px;font-weight:900;color:#fff;letter-spacing:-.02em;margin:20px 0 8px}
  .block-h2{font-size:16px;font-weight:700;color:rgba(255,255,255,.7);margin:16px 0 6px}
  .block-list{padding-left:20px;margin:8px 0;color:var(--muted);font-size:14px;line-height:1.75}
  .module-group{margin-bottom:28px}
  .module-group-header{display:flex;align-items:center;gap:12px;margin-bottom:14px}
  .module-group-badge{font-size:9px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;padding:4px 10px;border-radius:6px;background:var(--lime-soft);color:var(--lime);border:1px solid var(--lime-border)}
  .badge-revenue,.badge-operations,.badge-marketing,.badge-team,.badge-retention,.badge-agency{background:var(--lime-soft);color:var(--lime);border:1px solid var(--lime-border)}
  .module-group-title{font-family:var(--fd);font-size:13px;font-weight:700;color:rgba(255,255,255,.3)}
  .module-list{display:grid;grid-template-columns:1fr 1fr;gap:8px}
  .module-item{display:flex;align-items:flex-start;gap:10px;padding:12px 14px;background:#111;border:1px solid rgba(255,255,255,.06);border-radius:10px;page-break-inside:avoid;break-inside:avoid}
  .module-dot{width:6px;height:6px;border-radius:50%;background:var(--lime);opacity:.7;margin-top:5px;flex-shrink:0}
  .module-item-name{font-size:12px;font-weight:700;color:rgba(255,255,255,.75);margin-bottom:2px}
  .module-item-desc{font-size:11px;font-weight:400;color:rgba(255,255,255,.3);line-height:1.5}
  .widget-table{width:100%;border-collapse:collapse;margin:24px 0;font-size:13px}
  .widget-table thead tr{background:rgba(170,255,0,.08);border-bottom:1px solid var(--lime-border)}
  .widget-table thead th{padding:12px 16px;text-align:left;font-family:var(--fd);font-size:9px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:rgba(170,255,0,.6)}
  .widget-table tr.shaded{background:rgba(255,255,255,.02)}
  .widget-table tbody tr{border-bottom:1px solid rgba(255,255,255,.05)}
  .widget-table tbody td{padding:13px 16px;color:rgba(255,255,255,.4);font-weight:400;line-height:1.5}
  .widget-table tbody td:first-child{font-weight:600;color:rgba(255,255,255,.7)}
  .ownership-box{border-left:3px solid rgba(170,255,0,.4);background:rgba(170,255,0,.04);border:1px solid rgba(170,255,0,.08);border-left:3px solid rgba(170,255,0,.4);border-radius:0 8px 8px 0;padding:16px 20px;margin:28px 0}
  .ownership-box .ob-label{font-size:9px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:rgba(170,255,0,.45);margin-bottom:8px}
  .ownership-box p{font-size:13px;font-weight:400;color:var(--muted);line-height:1.7}
  .investment-table{width:100%;border-collapse:collapse;margin:24px 0}
  .investment-table thead tr{border-bottom:1px solid rgba(255,255,255,.08)}
  .investment-table thead th{padding:10px 16px 10px 0;text-align:left;font-size:9px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:rgba(255,255,255,.25)}
  .investment-table thead th:last-child{text-align:right;padding-right:0}
  .investment-table tbody td{padding:14px 16px 14px 0;border-bottom:1px solid rgba(255,255,255,.05);font-size:14px;color:rgba(255,255,255,.5);font-weight:400}
  .investment-table tbody td:last-child{text-align:right;font-weight:600;color:rgba(255,255,255,.8);padding-right:0}
  .investment-table tfoot td{padding:14px 16px 0 0;font-size:14px;font-weight:700;color:#fff}
  .investment-table tfoot td:last-child{text-align:right;padding-right:0;color:var(--lime);font-weight:900;font-size:16px}
  .investment-table tfoot tr:first-child td{border-top:1px solid rgba(255,255,255,.1)}
  .type-pill{display:inline-block;font-size:9px;font-weight:600;letter-spacing:.08em;padding:3px 8px;border-radius:2px;text-transform:uppercase;border:1px solid}
  .pill-once{background:rgba(170,255,0,.08);color:rgba(170,255,0,.8);border-color:var(--lime-border)}
  .pill-monthly{background:rgba(255,255,255,.05);color:var(--muted);border-color:rgba(255,255,255,.08)}
  .pill-addon{background:rgba(170,255,0,.08);color:rgba(170,255,0,.8);border-color:var(--lime-border)}
  .pill-client{background:rgba(255,255,255,.04);color:rgba(255,255,255,.3);border-color:rgba(255,255,255,.06)}
  .investment-note{font-size:12px;font-weight:400;color:rgba(255,255,255,.25);font-style:italic;margin-top:12px;page-break-before:avoid;break-before:avoid}
  ...
  .phase2-item{display:grid;grid-template-columns:200px 1fr auto;gap:24px;align-items:start;padding:20px 24px;border:1px solid rgba(255,255,255,.06);background:#111;border-radius:10px;page-break-inside:avoid;break-inside:avoid}
  .phase2-name{font-family:var(--fd);font-size:13px;font-weight:700;color:rgba(255,255,255,.75);margin-bottom:4px}
  .phase2-timing{font-size:9px;font-weight:600;letter-spacing:.08em;color:rgba(255,255,255,.2);text-transform:uppercase}
  .phase2-desc{font-size:12px;font-weight:400;color:rgba(255,255,255,.4);line-height:1.6}
  .phase2-price{text-align:right;white-space:nowrap}
  .phase2-price .amount{font-family:var(--fd);font-size:14px;font-weight:900;color:var(--lime);display:block;letter-spacing:-.02em}
  .phase2-price .cadence{font-size:10px;color:rgba(255,255,255,.2);font-weight:400}
  .steps-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:28px 0}
  .step-item{padding:20px;border:1px solid rgba(255,255,255,.06);background:#111;border-radius:10px;page-break-inside:avoid;break-inside:avoid}
  .step-number{font-family:var(--fd);font-size:44px;font-weight:900;color:rgba(170,255,0,.12);line-height:1;margin-bottom:12px;letter-spacing:-.04em}
  .step-title{font-family:var(--fd);font-size:13px;font-weight:700;color:rgba(255,255,255,.8);margin-bottom:8px}
  .step-desc{font-size:12px;font-weight:400;color:rgba(255,255,255,.35);line-height:1.65}
  .cta-block{background:#000;margin:48px -64px -60px;padding:48px 64px;display:grid;grid-template-columns:1fr auto;align-items:center;gap:32px;border-top:1px solid rgba(170,255,0,.1)}
  .cta-block h2{font-family:var(--fd);font-size:26px;font-weight:900;color:#fff;line-height:1.2;letter-spacing:-.03em}
  .cta-block h2 span{color:var(--lime)}
  .cta-block p{font-size:13px;font-weight:400;color:rgba(255,255,255,.3);margin-top:8px;line-height:1.6}
  .cta-contacts{display:flex;flex-direction:column;gap:10px;align-items:flex-end}
  .cta-contact-item{font-size:13px;color:rgba(255,255,255,.6);font-weight:500;text-align:right}
  .cta-contact-item label{display:block;font-size:8px;font-weight:700;letter-spacing:.14em;color:rgba(170,255,0,.4);text-transform:uppercase;margin-bottom:2px}
  .validity{font-size:10px;font-weight:400;color:rgba(255,255,255,.2);font-style:normal;text-align:center;padding:20px 64px;border-top:1px solid rgba(255,255,255,.05);background:var(--bg)}
  .addon-group-label{font-size:10px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:rgba(255,255,255,.2);margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid rgba(255,255,255,.05)}
  .addon-now-grid{display:grid;grid-template-columns:1fr;gap:10px;margin-bottom:8px}
  .addon-now-item{display:grid;grid-template-columns:1fr auto;gap:24px;align-items:start;padding:18px 20px;background:#111;border:1px solid rgba(255,255,255,.06);border-left:2px solid rgba(170,255,0,.4);border-radius:10px;page-break-inside:avoid;break-inside:avoid}
  .addon-now-name{font-family:var(--fd);font-size:12px;font-weight:700;color:rgba(255,255,255,.75);margin-bottom:4px}
  .addon-now-desc{font-size:12px;font-weight:400;color:rgba(255,255,255,.35);line-height:1.55}
  .addon-now-price{text-align:right;white-space:nowrap;flex-shrink:0}
  .addon-now-amount{font-family:var(--fd);font-size:13px;font-weight:900;color:var(--lime);display:block}
  .addon-now-cadence{font-size:10px;color:rgba(255,255,255,.2);font-weight:400}
`;
...
function renderProposal(data) {
  ...
  const situationHTML = situation.map((s, i) => {
    ...
    <p class="section-lead">${escape(text)}</p>`
  }).join('\n');
...
// These are the natural expansion paths + standard add-ons.
const OS_DEFAULT_ADDONS_LATER = {
  'Revenue OS':    ['Operations OS', 'Retention OS', 'Marketing OS', 'Enhanced Dashboard', 'Project Kickoff Automation', 'Document Generation', 'Lead Capture System', 'Client Portal View'],
  'Operations OS': ['Revenue OS', 'Team OS', 'Retention OS', 'Enhanced Dashboard', 'Project Kickoff Automation', 'Client Onboarding Kickoff', 'Document Generation', 'Client Portal View'],
  'Business OS':   ['Marketing OS', 'Team OS', 'Retention OS', 'Enhanced Dashboard', 'Project Kickoff Automation', 'Client Onboarding Kickoff', 'Document Generation', 'Lead Capture System', 'Client Portal View'],
  'Agency OS':     ['Team OS', 'Retention OS', 'Enhanced Dashboard', 'Project Kickoff Automation', 'Campaign Kickoff Automation', 'Client Onboarding Kickoff', 'Document Generation', 'Client Portal View'],
  'Marketing OS':  ['Revenue OS', 'Retention OS', 'Ads Platform Integration', 'Campaign Kickoff Automation', 'Enhanced Dashboard', 'Document Generation', 'Client Portal View'],
  'Team OS':       ['Revenue OS', 'Retention OS', 'Hiring Kickoff Automation', 'Enhanced Dashboard', 'Client Portal View'],
  'Retention OS':  ['Revenue OS', 'Marketing OS', 'Team OS', 'Renewal Kickoff Automation', 'Enhanced Dashboard', 'Document Generation', 'Client Portal View'],
};
