...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ── Page 2 — System ────────────────────────────────────────────────────── */
  .p2-header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:32px}
  .p2-title{font-family:var(--fh);font-size:28px;font-weight:700;color:var(--black);letter-spacing:-.02em;line-height:1}
  .p2-subtitle{font-size:12px;color:var(--muted);margin-top:8px;line-height:1.55;max-width:420px}
  .p2-total-badge{background:var(--black);color:#fff;padding:12px 18px;border-radius:4px;text-align:right;min-width:110px}
  .p2-total-label{font-family:var(--fh);font-size:8px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:rgba(255,255,255,.55);margin-bottom:5px}
  .p2-total-amount{font-family:var(--fh);font-size:18px;font-weight:700;color:#fff;white-space:nowrap}

  /* OS rows */
  .os-section{display:grid;grid-template-columns:1fr 100px;align-items:start;gap:24px;padding:20px 0;border-top:1px solid var(--border);page-break-inside:avoid;break-inside:avoid}
  .os-section:last-of-type{border-bottom:1px solid var(--border)}
  .os-tier-label{font-family:var(--fh);font-size:8px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--muted);margin-bottom:5px}
  .os-name{font-family:var(--fh);font-size:15px;font-weight:700;color:var(--black);margin-bottom:5px}
  .os-desc{font-size:11.5px;color:var(--muted);line-height:1.5;margin-bottom:12px}
  .module-pills{display:flex;flex-wrap:wrap;gap:4px}
  .module-pill{font-size:10px;color:var(--fg);background:#F2F2EF;padding:3px 8px;border-radius:2px;white-space:nowrap;line-height:1.5}
  .os-price-area{text-align:right}
  .os-price{font-family:var(--fh);font-size:15px;font-weight:700;color:var(--black);white-space:nowrap}
  .os-price-note{font-size:9.5px;color:var(--muted);margin-top:3px}

  /* Add-ons */
  .addons-wrapper{margin-top:28px}
  .addons-label{font-family:var(--fh);font-size:8.5px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--muted);margin-bottom:12px}
  .addon-item{display:grid;grid-template-columns:1fr auto;align-items:baseline;gap:16px;padding:10px 0;border-top:1px solid var(--border)}
  .addon-item:last-of-type{border-bottom:1px solid var(--border)}
  .addon-item-name{font-family:var(--fh);font-size:12px;font-weight:600;color:var(--black)}
  .addon-item-price{font-family:var(--fh);font-size:12px;font-weight:700;color:var(--black);white-space:nowrap;text-align:right}
  .addon-item-cad{font-size:9.5px;color:var(--muted);text-align:right;margin-top:2px}

  /* Also available later */
  .also-later{margin-top:28px}
  .also-later-label{font-family:var(--fh);font-size:8.5px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--muted);margin-bottom:10px}
  .also-later-pills{display:flex;flex-wrap:wrap;gap:5px}
  .also-later-pill{font-size:10.5px;color:var(--muted);border:1px solid #E0E0DA;padding:4px 10px;border-radius:20px;white-space:nowrap}

  /* ── Page 3 — Investment ────────────────────────────────────────────────── */
  .inv-heading{font-family:var(--fh);font-size:26px;font-weight:700;color:var(--black);margin-bottom:24px;letter-spacing:-.02em}
  .inv-table{width:100%;border-collapse:collapse;margin-bottom:0}
  .inv-table thead tr{border-bottom:1.5px solid var(--black)}
  .inv-table thead th{font-family:var(--fh);font-size:8.5px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);padding:0 0 10px;text-align:left}
  .inv-table thead th:last-child{text-align:right}
  .inv-table tbody td{padding:13px 0;border-bottom:1px solid var(--border);vertical-align:top}
  .inv-table tbody td:last-child{text-align:right;white-space:nowrap;vertical-align:middle}
  ...
  .inv-item-name{font-family:var(--fh);font-size:13px;font-weight:600;color:var(--black);margin-bottom:3px}
  .inv-item-type{font-size:9.5px;color:var(--muted);margin-top:2px}
  .inv-item-desc{font-size:11px;color:var(--muted);line-height:1.5;margin-top:3px}
  .inv-amount{font-family:var(--fh);font-size:14px;font-weight:700;color:var(--black)}
  .inv-amount-comp{font-family:var(--fh);font-size:12px;font-weight:700;color:#5A8A00;background:#F0FFD6;padding:3px 9px;border-radius:20px;display:inline-block;white-space:nowrap}
  .inv-note{font-size:11.5px;color:var(--muted);font-style:italic;margin-top:18px;line-height:1.65;padding-top:16px;border-top:1px solid var(--border)}
  .inv-terms{display:grid;grid-template-columns:1fr 1fr;gap:16px 24px;margin-top:20px;padding-top:20px;border-top:1px solid var(--border)}
  .inv-term-item label{font-family:var(--fh);font-size:8px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);display:block;margin-bottom:5px}
  .inv-term-item span{font-size:12.5px;color:var(--fg);font-weight:500}
...
<div class="page">
  <div class="page-body">
    ...
    <div class="os-section">
      <div>
        <div class="os-tier-label">Foundation — Complimentary</div>
        <div class="os-name">Base OS</div>
        <div class="module-pills">${BASE_OS_MODULES.map(m => `<span class="module-pill">${escape(m)}</span>`).join('')}</div>
      </div>
      <div class="os-price-area">
        <div class="os-price">—</div>
        <div class="os-price-note">Always included</div>
      </div>
    </div>
    ...
    ${osTypes.length ? osTypes.map(osName => {
      ...
      return `<div class="os-section">
        <div>
          <div class="os-tier-label">OS Layer</div>
          <div class="os-name">${escape(osName)}</div>
          ${desc ? `<div class="os-desc">${escape(desc)}</div>` : ''}
          <div class="module-pills">${mods.map(m => `<span class="module-pill">${escape(m)}</span>`).join('')}</div>
        </div>
        <div class="os-price-area">
          <div class="os-price">${price ? fmt(price) : (osTypes.length === 1 && coreFee ? fmt(coreFee) : '—')}</div>
          <div class="os-price-note">One-time</div>
        </div>
      </div>`;
    }).join('') : `<div class="os-section"><div class="os-name" style="color:var(--muted);font-style:italic">System scope confirmed on discovery call.</div></div>`}

    <!-- Add-ons included -->
    ${addons_now.length ? `
    <div class="addons-wrapper">
      <div class="addons-label">Add-Ons — Included in This Proposal</div>
      ${addons_now.map(item => {
        const isObj = typeof item === 'object' && item !== null;
        const name  = isObj ? (item.name || '') : String(item);
        const lib   = ADDON_LIBRARY[name] || {};
        ...
        return `<div class="addon-item">
          <div class="addon-item-name">${escape(name)}</div>
          <div style="text-align:right">
            <div class="addon-item-price">${escape(price || '—')}</div>
            <div class="addon-item-cad">${escape(cad)}</div>
          </div>
        </div>`;
      }).join('')}
    </div>` : ''}
    ...
    <!-- Also available -->
    ${addons_later.length ? `
    <div class="also-later">
      <div class="also-later-label">${addons_now.length ? 'Also Available Any Time' : 'Available Any Time'}</div>
      <div class="also-later-pills">${addons_later.map(n => `<span class="also-later-pill">${escape(n)}</span>`).join('')}</div>
    </div>` : ''}
    ...
    <table class="inv-table">
      ...
      <tbody>
        <tr>
          ...
          <td><span class="inv-amount-comp">Complimentary</span></td>
        </tr>
        ${osTypes.map(osName => {
          const mods = modules[osName] || [];
          return `<tr>
            <td>
              ...
              <div class="inv-item-type">One-time installation</div>
            </td>
            <td><div class="inv-amount">${coreFee && osTypes.length === 1 ? fmt(coreFee) : '—'}</div></td>
          </tr>`;
        }).join('')}
        ${addons_now.map(item => {
          const isObj = typeof item === 'object' && item !== null;
          ...
          return `<tr>
            <td>
              <div class="inv-item-name">${escape(name)}</div>
              <div class="inv-item-type">${escape(cad || 'One-time')}</div>
            </td>
            <td><div class="inv-amount">${escape(price || '—')}</div></td>
          </tr>`;
        }).join('')}
        <tr>
          <td>
            <div class="inv-item-name">Notion ${escape(notion_plan)} Plan</div>
            <div class="inv-item-type">Monthly · billed directly to your workspace</div>
          </td>
          <td><div class="inv-amount">~RM 50/mo</div></td>
        </tr>
