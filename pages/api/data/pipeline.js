...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const lead of leads) {
      ...
      const leadVal    = p["Potential Value (MYR)"]?.number || p["Potential Value"]?.number || p["Estimated Value"]?.number || p["Deal Value (MYR)"]?.number || p["Value"]?.number || p["Deal Value"]?.number || 0
