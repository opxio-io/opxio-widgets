...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ── Page 2 — System ────────────────────────────────────────────────────── */
  .sys-header{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:32px}
  .sys-heading{font-family:var(--fh);font-size:26px;font-weight:700;color:var(--black);letter-spacing:-.01em;margin-bottom:6px}
  .sys-total{font-family:var(--fh);font-size:22px;font-weight:700;color:var(--black);white-space:nowrap;padding-left:24px}
  .sys-total-label{font-size:10px;color:var(--muted);text-align:right;margin-bottom:4px;letter-spacing:.06em;text-transform:uppercase}
  .sys-subtitle{font-size:12px;color:var(--muted);line-height:1.6;max-width:440px}

  /* OS card — bordered container */
  .os-card{border:1px solid #DDDDD8;border-radius:6px;padding:18px 20px;margin-bottom:10px;display:grid;grid-template-columns:1fr auto;align-items:start;gap:20px;page-break-inside:avoid;break-inside:avoid}
  .os-card-base{background:#F9F9F7;border-color:#E8E8E2}
  .os-card-body{}
  .os-name{font-family:var(--fh);font-size:13.5px;font-weight:700;color:var(--black);margin-bottom:3px}
  .os-badge{font-size:9.5px;color:var(--muted);font-style:italic;margin-left:8px;font-weight:400;letter-spacing:.02em}
  .os-desc{font-size:11.5px;color:var(--muted);margin-bottom:10px;line-height:1.5}
  .os-price-col{text-align:right;min-width:80px}
  .os-price{font-family:var(--fh);font-size:14px;font-weight:700;color:var(--black);white-space:nowrap}
  .os-price-note{font-size:9.5px;color:var(--muted);margin-top:2px}
  .modules-label{font-family:var(--fh);font-size:8px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin-bottom:5px}
  .modules-inline{font-size:11.5px;color:var(--fg);line-height:1.7}
  .modules-inline span:not(:last-child)::after{content:' ·';color:var(--border)}

  /* Add-ons */
  .addons-section-heading{font-family:var(--fh);font-size:9px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin:28px 0 14px}
  .addon-row{display:grid;grid-template-columns:1fr auto;align-items:baseline;gap:16px;padding:9px 0;border-bottom:1px solid var(--border)}
  .addon-row:last-child{border-bottom:none}
  .addon-row-name{font-family:var(--fh);font-size:12px;font-weight:600;color:var(--black)}
  .addon-row-desc{font-size:10.5px;color:var(--muted);margin-top:2px;line-height:1.5}
  .addon-row-price{font-family:var(--fh);font-size:12px;font-weight:700;color:var(--black);white-space:nowrap;text-align:right}
  .addon-row-cad{font-size:9.5px;color:var(--muted);text-align:right}
  .also-avail-label{font-family:var(--fh);font-size:9px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin-top:20px;margin-bottom:8px}
  .also-avail-list{font-size:11.5px;color:var(--muted);line-height:1.9}
  ...
  <div class="page-body">
    ...
    <div class="sys-header">
      <div>
        ...
        <div class="sys-subtitle" style="margin-top:6px">Every component connects into one system built around how ${escape(company_name)} actually operates.</div>
      </div>
      ${coreFee ? `<div style="text-align:right">
        <div class="sys-total-label">Total Investment</div>
        <div class="sys-total">${fmt(coreFee)}</div>
      </div>` : ''}
    </div>

    <!-- Base OS — always complimentary -->
    <div class="os-card os-card-base">
      <div class="os-card-body">
        <div class="os-name">Base OS <span class="os-badge">Complimentary</span></div>
        <div class="os-desc" style="margin-top:4px">Foundation layer included in every install.</div>
        <div class="modules-label" style="margin-top:10px">Modules Included</div>
        <div class="modules-inline">${BASE_OS_MODULES.map(m => `<span>${escape(m)}</span>`).join('')}</div>
      </div>
    ...
    ${osTypes.length ? osTypes.map(osName => {
      ...
      return `<div class="os-card">
        <div class="os-card-body">
          <div class="os-name">${escape(osName)}</div>
          ${desc ? `<div class="os-desc" style="margin-top:4px">${escape(desc)}</div>` : ''}
          <div class="modules-label" style="margin-top:10px">Modules Included</div>
          <div class="modules-inline">${mods.map(m => `<span>${escape(m)}</span>`).join('')}</div>
        </div>
        <div class="os-price-col">
    ...
    }).join('') : `<div class="os-card"><div class="os-name" style="color:var(--muted);font-style:italic">System scope confirmed on discovery call.</div></div>`}

    <!-- Add-ons in this proposal -->
    ${addons_now.length ? `
    <div class="addons-section-heading">Add-Ons — Included in This Proposal</div>
    ${addons_now.map(item => {
      const isObj = typeof item === 'object' && item !== null;
      const name  = isObj ? (item.name || '') : String(item);
      const lib   = ADDON_LIBRARY[name] || {};
      ...
      return `<div class="addon-row">
        <div>
          <div class="addon-row-name">${escape(name)}</div>
          ${desc ? `<div class="addon-row-desc">${escape(desc)}</div>` : ''}
        </div>
        <div style="text-align:right">
          <div class="addon-row-price">${escape(price || '—')}</div>
          <div class="addon-row-cad">${escape(cad)}</div>
        </div>
      </div>`;
    }).join('')}` : ''}
    ...
    <!-- Also available -->
    ${addons_later.length ? `
    <div class="also-avail-label">${addons_now.length ? 'Also Available Any Time' : 'Available Any Time'}</div>
    <div class="also-avail-list">${addons_later.map(n => `<span style="white-space:nowrap">${escape(n)}</span>`).join('<span style="color:var(--border);margin:0 5px">·</span>')}</div>` : ''}
